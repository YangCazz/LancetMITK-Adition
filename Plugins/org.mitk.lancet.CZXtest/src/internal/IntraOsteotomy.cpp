...
vtkSmartPointer<vtkMatrix4x4> lancetAlgorithm::IntraOsteotomy::CalculateTDrillEnd2FemurImage(/*Eigen::Vector3d pointInImage*/)
{
	...
	TBase2BaseRF->DeepCopy(PKAData::m_TBase2BaseRF);
...
vtkSmartPointer<vtkMatrix4x4> lancetAlgorithm::IntraOsteotomy::CalculateTDrillEnd2FemurImage2()
{
	...
	TDrill2Camera->DeepCopy(PKAData::m_TCmaera2Drill);
	...
	vtkSmartPointer<vtkTransform> transform = vtkSmartPointer<vtkTransform>::New();
	transform->PreMultiply();
	transform->SetMatrix(TDrillEnd2Drill);
	transform->Concatenate(TDrill2Camera);
	transform->Concatenate(TCamera2FemurRF);
	transform->Concatenate(TFemurRF2FemurImage);
	transform->Update();
...
vtkSmartPointer<vtkMatrix4x4> lancetAlgorithm::IntraOsteotomy::CalculateTBase2DrillEnd()
{
	...
	TCamera2EndRF->DeepCopy(PKAData::m_TCmaera2Drill);
...
vtkSmartPointer<vtkMatrix4x4> lancetAlgorithm::IntraOsteotomy::CalculateDrillEndVerticalPlane(Eigen::Vector3d pointInPlane, Eigen::Vector3d planeNormal)
{
	vtkSmartPointer<vtkMatrix4x4> tDrillEnd2FemurImage = vtkSmartPointer<vtkMatrix4x4>::New();
	...
	Eigen::Vector3d planeNormalInDrill = CalculationHelper::TransformByMatrix(planeNormal, tDrillEnd2FemurImage);

	planeNormalInDrill.normalize();
	Eigen::Vector3d drillEndXaxis = Eigen::Vector3d(1, 0, 0);
	Eigen::Vector3d axis = drillEndXaxis.cross(planeNormalInDrill);

	// Compute the angle to rotate from a to -b 
	double dotProduct = drillEndXaxis.dot(planeNormalInDrill);
	if (dotProduct < 0) {
		// 如果法线与X轴反向，则翻转法线
		planeNormalInDrill = -planeNormalInDrill;
		dotProduct = -dotProduct; // 更新点积
	}
	...
	Eigen::Vector3d tranlation = pointInPlane;
	tranlation[0] = tranlation[0] - 50;
	
	return CalculationHelper::GetMatrixByRotationAndTranslation(rotationMatrix, tranlation);
}
	...
	Eigen::Vector3d pointInPlane, Eigen::Vector3d planeNormal)
{
	...
	// 设置旋转矩阵部分
	localCoordinateSystem->SetElement(0, 0, xAxis[0]);
	localCoordinateSystem->SetElement(1, 0, xAxis[1]);
	localCoordinateSystem->SetElement(2, 0, xAxis[2]);

	localCoordinateSystem->SetElement(0, 1, yAxis[0]);
	localCoordinateSystem->SetElement(1, 1, yAxis[1]);
	localCoordinateSystem->SetElement(2, 1, yAxis[2]);

	localCoordinateSystem->SetElement(0, 2, zAxis[0]);
	localCoordinateSystem->SetElement(1, 2, zAxis[1]);
	localCoordinateSystem->SetElement(2, 2, zAxis[2]);

	// 设置平移矩阵部分（变换的原点）
	localCoordinateSystem->SetElement(0, 3, origin[0]);
	localCoordinateSystem->SetElement(1, 3, origin[1]);
	localCoordinateSystem->SetElement(2, 3, origin[2]);
...
double lancetAlgorithm::IntraOsteotomy::GetDrillTip2DrillPlaneDistance(Eigen::Vector3d planePoint, Eigen::Vector3d planeNormal)
{
	return 0.0;
	Eigen::Vector3d tipPos = m_PKADianaAimHardwareDevice->GetDrillEndInCamera();
	//CalculationHelper::DistanceFromPointToPlane(tipPos, )
}
