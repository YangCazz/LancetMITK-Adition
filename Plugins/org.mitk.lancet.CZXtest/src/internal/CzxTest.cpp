...
void CzxTest::CreateQtPartControl(QWidget* parent)
{
	...
	connect(m_Controls.mitkNodeSelectWidget_landmark_src, &QmitkSingleNodeSelectionWidget::CurrentSelectionChanged,
		this, &CzxTest::OnNodeSelectionChanged);
	connect(m_Controls.mitkNodeSelectWidget_surface_regis, &QmitkSingleNodeSelectionWidget::CurrentSelectionChanged,
		this, &CzxTest::OnNodeSelectionChanged);
	...
	connect(m_Controls.IntraFemurVaAnglePlusBtn, &QPushButton::clicked, this, [this]()
		{IntraVaAngleChangeBtnClicked(true, KneeModel::Femur);
		});
	connect(m_Controls.IntraFemurVaAngleMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraVaAngleChangeBtnClicked(false, KneeModel::Femur);
		});

	connect(m_Controls.IntraFemurTiltAnglePlusBtn, &QPushButton::clicked, this, [this]()
		{IntraTiltAngleChangeBtnClicked(true, KneeModel::Femur);
		});
	connect(m_Controls.IntraFemurTiltAngleMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraTiltAngleChangeBtnClicked(false, KneeModel::Femur);
		});
	connect(m_Controls.IntraFemurRotationAnglePlusBtn, &QPushButton::clicked, this, [this]()
		{IntraRotationAngleChangeBtnClicked(true, KneeModel::Femur);
		});
	connect(m_Controls.IntraFemurRotationAngleMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraRotationAngleChangeBtnClicked(false, KneeModel::Femur);
		});
	connect(m_Controls.IntraFemurProudPlusBtn, &QPushButton::clicked, this, [this]()
		{IntraProudChangeBtnClicked(false, KneeModel::Femur);
		});
	connect(m_Controls.IntraFemurProudMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraProudChangeBtnClicked(true, KneeModel::Femur);
		});
	/*****************************************************************/
	connect(m_Controls.IntraTibiaVaAnglePlusBtn, &QPushButton::clicked, this, [this]()
		{IntraVaAngleChangeBtnClicked(true, KneeModel::Tibia);
		});
	connect(m_Controls.IntraTibiaVaAngleMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraVaAngleChangeBtnClicked(false, KneeModel::Tibia);
		});

	connect(m_Controls.IntraTibiaTiltAnglePlusBtn, &QPushButton::clicked, this, [this]()
		{IntraTiltAngleChangeBtnClicked(true, KneeModel::Tibia);
		});
	connect(m_Controls.IntraTibiaTiltAngleMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraTiltAngleChangeBtnClicked(false, KneeModel::Tibia);
		});
	connect(m_Controls.IntraTibiaRotationAnglePlusBtn, &QPushButton::clicked, this, [this]()
		{IntraRotationAngleChangeBtnClicked(true, KneeModel::Tibia);
		});
	connect(m_Controls.IntraTibiaRotationAngleMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraRotationAngleChangeBtnClicked(false, KneeModel::Tibia);
		});
	connect(m_Controls.IntraTibiaProudPlusBtn, &QPushButton::clicked, this, [this]()
		{IntraProudChangeBtnClicked(false, KneeModel::Tibia);
		});
	connect(m_Controls.IntraTibiaProudMinusBtn, &QPushButton::clicked, this, [this]()
		{IntraProudChangeBtnClicked(true, KneeModel::Tibia);
		});
...
void CzxTest::StartTrackingBtnClicked()
{
...
{
	m_PKADianaAimHardwareDevice->CaptureRobot();
	//m_PKAHardwareDevice->CaptureRobot();
}

void CzxTest::ResetRobotRegistrationBtnClicked()
{
	m_PKADianaAimHardwareDevice->ResetRobotRegistration();
...
void CzxTest::SaveRobotRegistrationBtnClicked()
{
	...
	std::string baseToBaseRFFileName = "T_BaseToBaseRF.txt";
	std::string flangeToEndRFFileName = "T_FlangeToEndRF.txt";

	FileIO::SaveMatrix2File(FileIO::CombinePath(filename.toStdString(), baseToBaseRFFileName).string(), PKAData::m_TBase2BaseRF);
...
void CzxTest::ReuseRobotRegistationBtnClicked()
{
	...
	std::string baseToBaseRFFileName = "T_BaseToBaseRF.txt";
	std::string flangeToEndRFFileName = "T_FlangeToEndRF.txt";
	vtkSmartPointer<vtkMatrix4x4> baseToBaseRF = vtkSmartPointer<vtkMatrix4x4>::New();
	vtkSmartPointer<vtkMatrix4x4> flangeToEndRF = vtkSmartPointer<vtkMatrix4x4>::New();
	FileIO::ReadTextFileAsvtkMatrix(FileIO::CombinePath(filename.toStdString(), baseToBaseRFFileName).string(), baseToBaseRF);
	FileIO::ReadTextFileAsvtkMatrix(FileIO::CombinePath(filename.toStdString(), flangeToEndRFFileName).string(), flangeToEndRF);
	PKAData::m_TBase2BaseRF->DeepCopy(baseToBaseRF);
	PKAData::m_TFlange2EndRF->DeepCopy(flangeToEndRF);
	PrintDataHelper::CoutMatrix("PKAData::m_TBase2BaseRF", PKAData::m_TBase2BaseRF);
...
void CzxTest::OnNodeSelectionChanged()
{
	m_Controls.textBrowser->append("nodeChanged!");
...
void CzxTest::InitHardwareDeviceTabConnection()
{
	...
	connect(m_Controls.StartTrackingBtn, &QPushButton::clicked, this, &CzxTest::StartTrackingBtnClicked);
...
{
	//std::string hansRobotPath = "E:\\PKAModelData\\HansRobotFrame";
	////std::vector<std::string> list = FileIO::GetListSubdirectories(hansRobotPath);
	//std::vector<std::string> list=FileIO::GetPathFilesWithFileType(hansRobotPath, ".stl");
	//for (std::string item : list)
	//{
	//	PKARenderHelper::LoadSingleMitkFile(this->GetDataStorage(), FileIO::CombinePath(hansRobotPath, item).string(),item);
	//	//std::cout << item << std::endl;
	//}
	//Render3D();
	//for (std::string item : list)
	//{
	//	auto geo = this->GetDataStorage()->GetNamedNode(item)->GetData()->GetGeometry();
	//	auto matrix = geo->GetVtkMatrix();
	//	
	//	PrintDataHelper::CoutMatrix(item.c_str(), matrix);
	//}
	...
	JointPartDescription Joint1Description = { "Joint1", hansRobotPath + "Joint1.stl", {-180, 180}, {218,0,-90, 0 } };
	//JointPartDescription Joint2Description = { "Joint2", hansRobotPath + "Joint2.stl", {-180, 180}, {380,-115.75,0, 0 } };
	PKARenderHelper::LoadSingleMitkFile(this->GetDataStorage(), hansRobotPath + "Joint2Back2.stl", "Joint2");
	...
	for (int i = 0; i < partDescriptions.size(); ++i)
	{
		...
		std::cout << "alpha" << part.DH[2] << std::endl;
		std::cout << "theta" << part.DH[3] << std::endl;
		joints.push_back(std::move(joint));
	}
	PKARenderHelper::LoadSingleMitkFile(this->GetDataStorage(), baseDescription.FileName, baseDescription.Name);

	m_RobotFrame = new RobotFrame(this->GetDataStorage(), joints, base);
	m_RobotFrame->AddRobot();
	double angles[1] = { 0 };
	m_RobotFrame->UpdateJoints(angles);
	auto iRenderWindowPart = GetRenderWindowPart();
	QmitkRenderWindow* renderWindow = iRenderWindowPart->GetQmitkRenderWindow("3d");
	...
	vtkSmartPointer<vtkTransform> transform = vtkSmartPointer<vtkTransform>::New();
	transform->SetMatrix(this->GetDataStorage()->GetNamedNode("Joint2")->GetData()->GetGeometry()->GetVtkMatrix());
	Eigen::Vector3d joint2Center(0, 115.75, 598);
	transform->Translate(joint2Center.data());
	
	vtkSmartPointer<vtkMatrix4x4> matrix = vtkSmartPointer<vtkMatrix4x4>::New();
	transform->GetMatrix(matrix);
	PrintDataHelper::CoutMatrix("Joint2 Matrix", matrix);
	this->GetDataStorage()->GetNamedNode("Joint2")->GetData()->GetGeometry()->SetIndexToWorldTransformByVtkMatrix(matrix);
	double directionY[3] = { 0,1,0 };
	double directionX[3] = { 1,0,0 };
	double directionZ[3] = { 0,0,1 };
	PKARenderHelper::RotateModel(directionX, joint2Center.data(), this->GetDataStorage()->GetNamedNode("Joint2"), 90);
	PKARenderHelper::RotateModel(directionY, joint2Center.data(), this->GetDataStorage()->GetNamedNode("Joint2"), -90);
	/*PKARenderHelper::RotateModel(directionY, joint2Center.data(), this->GetDataStorage()->GetNamedNode("Joint2"),180);
	PKARenderHelper::RotateModel(directionX, joint2Center.data(), this->GetDataStorage()->GetNamedNode("Joint2"), 90);
	PKARenderHelper::RotateModel(directionZ, joint2Center.data(), this->GetDataStorage()->GetNamedNode("Joint2"), 90);*/
	auto currentJoint2Matrix = this->GetDataStorage()->GetNamedNode("Joint2")->GetData()->GetGeometry()->GetVtkMatrix();
	PrintDataHelper::CoutMatrix("currentJoint2Matrix: ", currentJoint2Matrix);

	vtkSmartPointer<vtkTransform> base2Joint2Transform = vtkSmartPointer<vtkTransform>::New();
	vtkSmartPointer<vtkMatrix4x4> TJoint12Base = vtkSmartPointer<vtkMatrix4x4>::New();
	vtkSmartPointer<vtkMatrix4x4> TJoint12Joint2 = vtkSmartPointer<vtkMatrix4x4>::New();
	TJoint12Base->DeepCopy(this->GetDataStorage()->GetNamedNode("Joint1")->GetData()->GetGeometry()->GetVtkMatrix());
	TJoint12Base->Invert();
	base2Joint2Transform->PreMultiply();
	base2Joint2Transform->SetMatrix(TJoint12Base);
	base2Joint2Transform->Concatenate(currentJoint2Matrix);
	base2Joint2Transform->GetMatrix(TJoint12Joint2);
	PrintDataHelper::CoutMatrix("TJoint12Joint2: ", TJoint12Joint2);
}
...
{
	 std::string hansRobotPath = "E:\\PKAModelData\\HansRobotFrame2\\";
	 auto surface = (mitk::Surface*)this->GetDataStorage()->GetNamedNode("Joint2")->GetData();
	 ...
	 FileIO::WritePolyDataAsSTL(transformedPolyData, hansRobotPath + "Joint2Back2.stl");
}
...
void CzxTest::SaveIntraDataBtnClicked()
{
	...
	TTibia2Tray->DeepCopy(CalculationHelper::ConcatenateMatrixs(TTibia2Image, TImage2TrayMatrix));

	TFemur2Implant->DeepCopy(CalculationHelper::ConcatenateMatrixs(TFemur2Image, TImage2Implant));
...
void CzxTest::UpdateBoneIntraDisplay()
{
	...
	TfemurImage2TIbiaLocal->DeepCopy(CalculationHelper::ConcatenateMatrixs(TFemurImage2TibiaImage, tibiaCoordinate));
	...
	TFemurImage2TibiaTray->DeepCopy(CalculationHelper::ConcatenateMatrixs(TFemurImage2TibiaImage, TTibiaImage2TibiaTray));
