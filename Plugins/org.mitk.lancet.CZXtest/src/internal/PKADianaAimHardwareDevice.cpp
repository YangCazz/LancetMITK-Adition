...
void lancetAlgorithm::PKADianaAimHardwareDevice::UpdateCamera()
{
	...
	if (rlt == AIMOOE_OK)//判断是否采集成功
	{
		do
		{
			...
			UpdateCameraToToolMatrix(prlt, "PKADrill", PKAData::m_TCmaera2Drill, m_Labels[5]);
...
{
	Translate(axis[0], axis[1], axis[2]);
}
...
void lancetAlgorithm::PKADianaAimHardwareDevice::RobotTransform(const double* matrix)
{
	vtkSmartPointer<vtkMatrix4x4> vtkMatrix = vtkSmartPointer<vtkMatrix4x4>::New();
...
//vtkSmartPointer<vtkMatrix4x4> lancetAlgorithm::PKADianaAimHardwareDevice::GetCurrentTCP()
//{
//	double pose[6] = {};
//	getTcpPos(pose, m_RobotIpAddress);
...
void lancetAlgorithm::PKADianaAimHardwareDevice::CapturePose(bool translationOnly)
{
	...
	//vtkSmartPointer<vtkMatrix4x4> TCamera2RobotEndRF = vtkSmartPointer<vtkMatrix4x4>::New();
	//TCamera2RobotEndRF->DeepCopy(PKAData::m_TCamera2EndRF);
	...
	//PrintDataHelper::CoutMatrix("TBaseRF2Camera: ", TBaseRF2Camera);
	//vtkSmartPointer<vtkTransform> transform = vtkSmartPointer<vtkTransform>::New();
	//vtkSmartPointer<vtkMatrix4x4> TBaseRF2EndRF = vtkSmartPointer<vtkMatrix4x4>::New();
	//transform->PreMultiply();
	//transform->SetMatrix(TBaseRF2Camera);
	//transform->Concatenate(TCamera2RobotEndRF);
	//transform->GetMatrix(TBaseRF2EndRF);

	//// add vtkMatrix as poses to the registration module
	//// Average the NavigationData from the camera
...
void lancetAlgorithm::PKADianaAimHardwareDevice::CaptureRobot()
{
	// 增加捕获姿态和更新UI的方法
...
void lancetAlgorithm::PKADianaAimHardwareDevice::ResetRobotRegistration()
{
...
void lancetAlgorithm::PKADianaAimHardwareDevice::RobotAutoMove()
{
	...
	else if (i > 1 && i <= 9)
	{
		...
		this->RobotTransform(robotTransform->GetData());
	}
}
...
Eigen::Vector3d lancetAlgorithm::PKADianaAimHardwareDevice::GetDrillEndInCamera()
{
	...
	Eigen::Vector3d drillEnd = CalculationHelper::TransformByMatrix(m_DrillEndPoints, PKAData::m_TCmaera2Drill);
	return drillEnd;
}
