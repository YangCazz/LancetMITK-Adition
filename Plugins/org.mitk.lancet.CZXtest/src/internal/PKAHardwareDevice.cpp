...
void lancetAlgorithm::PKAKukaVegaHardwareDevice::CaptureRobot()
{
	...
	if (m_RobotRegistration.PoseCount() < 5) {
		...
		PKAData::m_TBase2BaseRF->DeepCopy(vtkT_BaseToBaseRF);
...
{
	FileIO::ReadTextFileAsvtkMatrix(std::string(getenv("USERPROFILE")) + "\\Desktop\\save\\T_BaseToBaseRF.txt", PKAData::m_TBase2BaseRF);
	...
	matrixBaseRF2Base->DeepCopy(PKAData::m_TBase2BaseRF);
	matrixBaseRF2Base->Invert();
...
{
	std::ofstream base2BaseRFFile(std::string(getenv("USERPROFILE")) + "\\Desktop\\save\\T_BaseToBaseRF.txt");
	auto matrixBase2BaseRF = PKAData::m_TBase2BaseRF->GetData();
	for (int i = 0; i < 16; i++) {
		base2BaseRFFile << matrixBase2BaseRF[i];
		if (i != 15) {
			base2BaseRFFile << ",";
		}
		else {
	...
	base2BaseRFFile << std::endl;
	base2BaseRFFile.close();
