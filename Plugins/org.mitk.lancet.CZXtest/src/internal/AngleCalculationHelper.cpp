...
std::pair<Tilt, double> lancetAlgorithm::AngleCalculationHelper::CalculateLimbFlexion()
{
	...
	TFemurLocal2TibiaLocal->DeepCopy(CalculationHelper::ConcatenateMatrixs(TFemurLocal2FemurImage, TFemur2Tibia, TTibiaBone2TibiaLocal));
...
vtkSmartPointer<vtkMatrix4x4> lancetAlgorithm::AngleCalculationHelper::CalculateTFemur2Tibia()
{
	...
	TFemur2Tibia->DeepCopy(CalculationHelper::ConcatenateMatrixs(TFemur2FemurRF, TFemurRF2Camera, TCamera2TibiaRF, TTibiaRF2Tibia));
	return TFemur2Tibia;
}
...
double lancetAlgorithm::AngleCalculationHelper::CalculateCutPlaneDistance()
{
	...
	if (PKAData::m_LimbTiltAngle >= 85 &&PKAData::m_LimbTiltAngle <= 95)
	{
		...
		std::cout << "85<TiltAngle<95" << result << std::endl;
		if (tibiaTrayProximalVectorInFemurImage.dot(femurPosteriorCutDirection))
		{
			return result;
		}
		else
	...
	else if(PKAData::m_LimbTiltAngle>=175 && PKAData::m_LimbTiltAngle <= 185)
	{
		...
		if (tibiaTrayProximalVectorInFemurImage.dot(femurDistalCutDirection))
		{
			return result;
		}
		else
