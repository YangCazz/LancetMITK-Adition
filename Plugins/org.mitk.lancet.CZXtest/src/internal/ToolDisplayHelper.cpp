...
void lancetAlgorithm::ToolDisplayHelper::UpdateTool()
{
	...
	for (auto& tool : m_ToolSet) {
		...
		case PKAMarker::PKADrill:
		{
			...
			currentDrillEnd = CalculationHelper::TransformByMatrix(m_DrillEndPoints, PKAData::m_TCmaera2Drill);

			TCamera2Drill->DeepCopy(PKAData::m_TCmaera2Drill);
			...
			TImage2Drill->DeepCopy(CalculationHelper::ConcatenateMatrixs(TImage2RF, TRF2Camera, PKAData::m_TCmaera2Drill));
			TImage2DrillEnd->DeepCopy(CalculationHelper::ConcatenateMatrixs(TImage2RF, TRF2Camera, m_DrillEndMatrix));
		...
		case PKAMarker::PKAProbe:
		{
			...
			TImage2RF->Invert();
			TRF2Camera->Invert();
			TImage2ProbeTip->DeepCopy(CalculationHelper::ConcatenateMatrixs(TImage2RF, TRF2Camera, probeTipMatrix));
